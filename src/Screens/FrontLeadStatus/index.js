...
export const FrontLeadStatus = () => {
  ...
  useEffect(() => {
    document.title = 'Mt Records | Front Lead Status Management';
  ...
  const handleSubmit = (event) => {
    ...
    document.querySelector('.loaderBox').classList.remove("d-none");
    const LogoutData = localStorage.getItem('login');
    fetch(`${process.env.REACT_APP_API_URL}/public/api/admin/addFrontLeadStatus`,
      {
        method: 'POST',
        headers: {
          'Accept': 'application/json',
  ...
  const brandID = (unitID) => {
    document.querySelector('.loaderBox').classList.remove("d-none");
    const LogoutData = localStorage.getItem('login');
    fetch(`${process.env.REACT_APP_API_URL}/public/api/admin/getFrontLeadStatus?id=${unitID}`,
      {
        method: 'GET',
        headers: {
          'Accept': 'application/json',
  ...
  const handleEditSubmit = (event) => {
    ...
    fetch(`${process.env.REACT_APP_API_URL}/public/api/admin/editFrontLeadStatus`,
      {
        method: 'POST',
        headers: {
          'Accept': 'application/json',
  ...
  useEffect(() => {
    if (removeID) {
      const LogoutData = localStorage.getItem('login');
      document.querySelector('.loaderBox').classList.remove("d-none");
      fetch(`${process.env.REACT_APP_API_URL}/public/api/admin/deleteFrontLeadStatus`,
        {
          method: 'POST',
          headers: {
            'Accept': 'application/json',
    ...
    <>
      <DashboardLayout>
        <div className="container-fluid">
              ...
              <div className="dashCard">
                <div className="row mb-3 justify-content-between">
                  <div className="col-md-6 mb-2">
                    <h2 className="mainTitle">Front Lead Status</h2>
                  </div>
                  <div className="col-md-6 mb-2">
                    {/* <div className="addUser">
                      {permission?.brands.create === true ? <CustomButton text="Add Front Leads Status Name" variant='primaryButton' onClick={() => {
                        setUser(true)
                      }} /> : ""}
                    ...
                    <div className="addUser">
                      <CustomButton text="Add Front Lead Status" variant='primaryButton' onClick={() => {
                        setUser(true)
                      }} />
                      <CustomInput type="text" placeholder="Search Here..." value={inputValue} inputClass="mainInput" onChange={handleChange} />
                    </div>
          ...
          {/* Add Front Leads Status Name  */}

          <CustomModal show={addUser} close={() => { setUser(false) }} >
            <CustomInput
              label="Add Front Leads Status Name"
              type="text"
              placeholder="Add Front Leads Status Name"
              required
              name="name"
          ...
          <CustomModal show={editUser} close={() => { setEditUser(false) }} >
            <CustomInput
              label="Edit Front Leads Status Name"
              type="text"
              placeholder="Edit Front Leads Status Name"
              required
              name="name"
              labelClass='mainLabel'
              inputClass='mainInput'
