...
export const FrontRegionManagement = () => {
  ...
  useEffect(() => {
    document.title = 'Mt Records | Front Region Management';
  ...
  const handleSubmit = (event) => {
    ...
    document.querySelector('.loaderBox').classList.remove("d-none");
    const LogoutData = localStorage.getItem('login');
    fetch(`${process.env.REACT_APP_API_URL}/public/api/admin/addFrontRegionStatus`,
      {
        method: 'POST',
        headers: {
          'Accept': 'application/json',
  ...
  const brandID = (unitID) => {
    document.querySelector('.loaderBox').classList.remove("d-none");
    const LogoutData = localStorage.getItem('login');
    fetch(`${process.env.REACT_APP_API_URL}/public/api/admin/getFrontRegionStatus?id=${unitID}`,
      {
        method: 'GET',
        headers: {
          'Accept': 'application/json',
  ...
  const handleEditSubmit = (event) => {
    ...
    fetch(`${process.env.REACT_APP_API_URL}/public/api/admin/editFrontRegionStatus`,
      {
        method: 'POST',
        headers: {
          'Accept': 'application/json',
  ...
  useEffect(() => {
    if (removeID) {
      const LogoutData = localStorage.getItem('login');
      document.querySelector('.loaderBox').classList.remove("d-none");
      fetch(`${process.env.REACT_APP_API_URL}/public/api/admin/deleteFrontRegionStatus`,
        {
          method: 'POST',
          headers: {
            'Accept': 'application/json',
      ...
      <DashboardLayout>
        <div className="container-fluid">
              ...
              <div className="dashCard">
                <div className="row mb-3 justify-content-between">
                  <div className="col-md-6 mb-2">
                    <h2 className="mainTitle">Front Region Management</h2>
                  </div>
                  <div className="col-md-6 mb-2">
                    ...
                    <div className="addUser">
                      <CustomButton text="Add Front Region Name" variant='primaryButton' onClick={() => {
                        setUser(true)
                      }} />
                      <CustomInput type="text" placeholder="Search Here..." value={inputValue} inputClass="mainInput" onChange={handleChange} />
                    </div>
          ...
          <CustomModal show={addUser} close={() => { setUser(false) }} >
            <CustomInput
              label="Add Front Region Name"
              type="text"
              placeholder="Add Front Region Name"
              required
              name="name"
          ...
          <CustomModal show={editUser} close={() => { setEditUser(false) }} >
            <CustomInput
              label="Edit Front Region Name"
              type="text"
              placeholder="Edit Front Region Name"
              required
              name="name"
              labelClass='mainLabel'
              inputClass='mainInput'
